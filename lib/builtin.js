  ...
  getClientName(aliasId) {
    ...
    if (!this.generator.moduleClass.has(aliasId || this.aliasId)) {
      this.generator.moduleClass.set(aliasId || this.aliasId, {
        namespace,
        className,
        aliasName: this.generator.getAliasName(namespace, className, 'Tea')
      });
    }
    // const { namespace, className } = BuiltinModule[aliasId || this.aliasId];
    // const aliasName = this.generator.getAliasName(namespace, className, 'Tea');
    // this.generator.used.push(`${aliasName} = ${namespace}.${className}`);
    // return aliasName;
...
class Func extends Builtin {
  constructor(generator) {
    super(generator, 'TeaFunc');
  }

  sleep(ast, level, env) {
    const clientName = this.getClientName('TeaCore');
    if (env.isAsyncMode) {
  ...
  constructor(generator) {
    ...
    Object.keys(typesMap).forEach(key => {
      ...
      this[key] = function (ast, level, env) {
        const clientName = this.getClientName('TeaConverter');
