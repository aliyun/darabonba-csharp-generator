...
class JSON extends Builtin {
  ...
  stringify(ast, level, env) {
    ...
    this.generator.emit(`Common.ToJSONString`);
    this.generator.visitArgs(ast, level, env);
  }
  ...
  readPath(ast, level, env) {
    this.generator.used.push(`Newtonsoft.Json.Linq`);
    this.generator.emit('Common.ReadPath');
    this.generator.visitArgs(ast, level, env);
  }
}
...
class Func extends Builtin {
  ...
  sleep(ast, level, env) {
    if (env.isAsyncMode) {
      ...
      this.generator.used.push('System.Threading.Tasks');
    } else {
  ...
  isNull(ast, level, env) {
    ...
    // 写在核心库的Extension中，不直接写==null是因为形如if ((data == null))的两层括号的情况
