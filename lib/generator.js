...
class Visitor {
  ...
  getRealModelName(fullModelName, type = 'Models') {
    ...
    if (fullModelName !== 'Darabonba.Model') {
...
`;
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if (node.fieldValue.fieldType === 'array') {
        ...
        if (node.fieldValue.fieldItemType.nodes !== undefined) {
          ...
          this.emit('\n');
          this.emit(`public class ${subModelName} : Model {\n`, level);
  ...
  visitAPIBody(ast, level, env) {
    ...
    this.emit(`Request ${REQUEST} = new Request();\n`, level);
  ...
  visitRuntimeBefore(ast, level, env) {
    ...
    this.emit('Request _lastRequest = null;\n', level);
    this.emit('Response _lastResponse = null;\n', level);
    ...
    this.emit('{\n', level + 1);
    this.emit(`long backoffTime = Core.GetBackoffDelay((${retryType})${RUNTIME}["retryOptions"], _retryPolicyContext);\n`, level + 2);
  ...
  visitEcxceptionBody(ast, level, exceptionName, env) {
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if (node.fieldValue.type === 'modelBody') {
        this.emit(`public class ${exceptionName}${_upperFirst(node.fieldName.lexeme)} : Model\n`, level);
        this.emit('{\n', level);
  ...
  eachAPI(ast, level, env) {
    ...
    if (env.isAsyncMode) {
      this.emit(`Response ${RESPONSE} = await Core.DoActionAsync(${REQUEST}`, baseLevel + 1);
    } else {
      this.emit(`Response ${RESPONSE} = Core.DoAction(${REQUEST}`, baseLevel + 1);
    }
