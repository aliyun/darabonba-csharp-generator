...
class Visitor {
  ...
  getExceptionName(ast, end) {
    let [notes] = DSL.note.getNotes(ast.notes, 0, end).slice(-1);
    if (notes && notes.note.lexeme === '@aliasName') {
      return _string(notes.arg.value);
    }
  }
  ...
  getRealModelName(fullModelName) {
    ...
    if (fullModelName !== 'Darabonba.DaraModel') {
...
`;
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if (node.fieldValue.fieldType === 'array') {
        ...
        if (node.fieldValue.fieldItemType.nodes !== undefined) {
          ...
          this.emit('\n');
          this.emit(`public class ${subModelName} : DaraModel {\n`, level);
  ...
  visitModelValidate(ast, modelName, level) {
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if (required || maxLength > 0 || minLength > 0 || maximum > 0 || pattern !== '') {
        ...
        if (pattern !== '') {
          this.emit(`${baseModel}.ValidatePattern("${attrName}", ${paramName}, "${pattern}");\n`, level);
        }
  ...
  visitRuntimeBefore(ast, level, env) {
    ...
    this.emit(`Darabonba.Models.RuntimeOptions ${RUNTIME} = `, level);
    this.visitObject(ast, level, env);
    this.emit(';\n');
    this.emit('\n');
    this.emit('RetryPolicyContext retryPolicyContext = null;\n', level);
    ...
    this.emit('retryPolicyContext = new RetryPolicyContext\n', level);
    this.emit('{\n', level);
    this.emit('RetriesAttempted = _retriesAttempted\n', level + 1);
    this.emit('}\n', level);
    this.emit(`while (DaraCore.ShouldRetry(${RUNTIME}["retryOptions"], retryPolicyContext))\n`, level);
    this.emit('{\n', level);
    this.emit('if (_retriesAttempted > 0)\n', level + 1);
    this.emit('{\n', level + 1);
    this.emit(`int backoffTime = DaraCore.GetBackoffDelay(${RUNTIME}["retryOptions"], retryPolicyContext);\n`, level + 2);
  ...
  visitConstructModel(ast, level, env) {
    ...
    if (ast.aliasId.isModule) {
      ...
      if (_isBuiltinModel(modelName)) {
        const subModelName = this.getRealModelName((modelName));
        this.emit(subModelName);
      } else {
  ...
  visitEcxceptionBody(ast, level, exceptionName, env) {
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if (node.fieldValue.type === 'modelBody') {
        this.emit(`public class ${exceptionName}${_upperFirst(node.fieldName.lexeme)} : DaraModel\n`, level);
        this.emit('{\n', level);
  ...
  visitExceptions(ast, filepath, level) {
    ...
    for (let i = 0; i < exceptions.length; i++) {
      ...
      const realExceptionName = this.getExceptionName(ast, exceptions[i].tokenRange[0]);
