...
function _isSuperField(field) {
  const fields = [
    'code', 'message', 'data', 'statusCode',
    'description', 'accessDeniedDetail', 'dataResult'
  ];
...
class Visitor {
  ...
  getRealModelName(fullModelName) {
    ...
    const namespace = fullModelName.replace(/\.[^.]*$/, '');
    ...
    const existName = this.usedClass.get(modelName);
    if (existName && existName !== namespace) {
      return `${fullModelName}`;
    } else if (this.conflictModelNameMap.includes(modelName)) {
      return `${fullModelName}`;
    }

    this.used.push(`${(this.getType((namespace)))}`);
    this.usedClass.set(modelName, namespace);
    return _type(modelName);
  }

  getType(name) {
    if (name === 'TeaRequest' || name === 'TeaResponse' ||
      name === 'TeaURL' || name === 'TeaStream' || name === 'TeaDate' ||
      name === 'TeaModel' || name === 'TeaFile' || name === 'TeaForm' ||
      name === 'TeaNumber' || name === 'TeaString' || name === 'TeaXML' ||
      name === 'TeaException' || name === '$Request' || name === '$Model' ||
      name === '$Response' || name === '$URL' || name === '$Stream' || name === '$Date' ||
      name === '$Model' || name === '$File' || name === '$Form' ||
      name === '$Number' || name === '$String' || name === '$XML' ||
      name === '$Error') {
      return 'Tea';
    }
    return name;
...
`;
  ...
  visitModule(ast, filepath, main, level) {
    ...
    if (this.config.packageInfo.interface) {
      ...
      if (extendParam.modelsCount && extendParam.modelsCount > 0) {
        // TODO 直接这样push是有问题的
        this.used.push(`${this.namespace}.Models`);
      }
  ...
  visitModelBody(ast, level, env, modelName) {
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if (node.fieldValue.fieldType === 'array') {
        ...
        if (node.fieldValue.fieldType.idType === 'module') {
          ...
          this.emit(`${node.fieldValue.lexeme}`);
    ...
    this.emit(`public new ${modelName} Copy()\n`, level);
    this.emit('{\n', level);
    this.visitModelCopy(modelName, level + 1);
    this.emit('}\n', level);
    this.emit('\n');

    this.emit(`public new ${modelName} CopyWithoutStream()\n`, level);
    this.emit('{\n', level);
    ...
    this.emit('public new Dictionary<string, object> ToMap(bool noStream = false)\n', level);
    this.emit('{\n', level);
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      this.emit(`if (${_upperFirst(_name(node.fieldName))} != null)\n`, level + 1);
      this.emit(`{\n`, level + 1);
      const fieldName = _avoidReserveName(_upperFirst(_name(node.fieldName)));
      const depth = 1;
    ...
    this.emit(`public static new ${modelName} FromMap(Dictionary<string, object> map)\n`, level);
    this.emit(`{\n`, level);
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      this.emit(`{\n`, level + 1);
      this.visitFromField(node.fieldValue, modelName, fieldName, `model.${fieldName}`, `map["${realName}"]`, level + 1);
      // this.visitFromField(node.fieldValue, modelName, fieldName, `map["${fieldName}"]`, `map["${realName}"]`, type, level + 1);
      this.emit('}\n', level + 1);
      this.emit('\n');
    }
  ...
  visitModelValidate(ast, level) {
    const modelName = 'TeaModel';
    for (let i = 0; i < ast.nodes.length; i++) {
      const node = ast.nodes[i];
      this.visitFieldValidate(modelName, node.fieldValue, level, `${_upperFirst(_name(node.fieldName))}`);
      const attrName = _upperFirst(_name(node.fieldName));
      ...
      if (required || maxLength > 0 || minLength > 0 || maximum > 0 || pattern !== '') {
        if (required) {
          this.emit(`${modelName}.ValidateRequired("${attrName}", ${attrName}, true);\n`, level);
        }
        if (pattern !== '') {
          this.emit(`${modelName}.ValidatePattern("${attrName}", ${attrName}, "${pattern}");\n`, level);
        }
        if (maxLength > 0 && maxLength <= 2147483647) {
          this.emit(`${modelName}.ValidateMaxLength("${attrName}", ${attrName}, ${maxLength});\n`, level);
        }
        if (minLength > 0 && minLength <= 2147483647) {
          this.emit(`${modelName}.ValidateMinLength("${attrName}", ${attrName}, ${minLength});\n`, level);
        }
        // 不能超过JS中最大安全整数
        if (maximum > 0 && maximum <= Number.MAX_SAFE_INTEGER) {
          this.emit(`${modelName}.ValidateMaximum("${attrName}", ${attrName}, ${maximum});\n`, level);
        }
        // 不能超过JS中最大安全整数
        if (minimum > 0 && minimum <= Number.MAX_SAFE_INTEGER) {
          this.emit(`${modelName}.ValidateMinimum("${attrName}", ${attrName}, ${minimum});\n`, level);
        }
      }
    }
  ...
  visitFromField(fieldValue, modelName, fieldName, key, value, level) {
    ...
    if (fieldValue.type === 'modelBody') {
      ...
      this.emit(`var dict = map["${fieldName}"] as Dictionary<string, ${valueType}>;\n`, level + 1);
  ...
  visitToMapField(fieldValue, modelName, fieldName, key, value, depth, level) {
    ...
    if (fieldValue.type === 'modelBody' || fieldValue.type === 'moduleModel' ||
      ...
      this.emit(`${key} = ${value} != null ? ${value}.ToMap(noStream) : null;\n`, level + 1);
    ...
    if (ast.type === 'array') {
      ...
      models.forEach((item, index) => {
        ...
        if (index === 0) {
          ...
          const realModelName = this.getRealModelName(`${namespace}.Models.${tempName}`);
          this.emit(realModelName);
        } else {
          tempName += _upperFirst(_type(item));
          this.emit(tempName);
      ...
      const realModelName = this.getRealModelName(`${this.namespace}.Models.${_upperFirst(ast.lexeme)}`);
      this.emit(realModelName);
    } else if (ast.idType === 'builtin_model') {
      const realModelName = this.getRealModelName(_type(ast.lexeme));
      this.emit(realModelName);
  ...
  visitFieldType(value, level, modelName, fieldName) {
    if (value.type === 'modelBody') {
      ...
      const realModelName = this.getRealModelName(`${this.namespace}.Models.${subModelName}`);
      this.emit(realModelName);
    } else if (typeof value.fieldType === 'string') {
      this.emit(`${_type(value.fieldType)}`);
      ...
      const realModelName = this.getRealModelName(_type(value.fieldType.lexeme));
      this.emit(realModelName);
    }
  }
  ...
  visitConstructModel(ast, level, env) {
    ...
    if (ast.aliasId.isModule) {
      ...
      if (_isBuiltinModel(modelName)) {
        const subModelName = this.getRealModelName(_type(modelName));
        this.emit(subModelName);
      } else {
        const subModelName = this.getRealModelName(`${this.namespace}.Models.${modelName}`);
        this.emit(subModelName);
      }
    } else {
    ...
    if (ast.object && ast.object.fields && ast.object.fields.length > 0) {
      ...
      let comments = DSL.comment.getBackComments(this.comments, ast.object.fields[i - 1].tokenRange[1]);
      this.visitComments(comments, level + 1);
      this.emit('}', level);
  ...
  visitExtendOn(extendOn, type = 'model') {
    if (!extendOn) {
      return type === 'model' ? this.emit(`TeaModel`) : this.emit(`TeaException`);
    }
    let namespace = this.namespace;
    ...
    if (extendOn.type === 'moduleModel') {
      ...
      this.emit(this.getRealModelName(`${namespace}.Models.${modelName}`));
    }
  }
  ...
  visitEcxceptionBody(ast, level, ExceptionName) {
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      // TODO 覆盖TeaException已有的字段时加new，但是如果是继承的父类的而不是Teaexception的怎么加new? 不加new不报错，报warning
      if (_isSuperField(_escape(_name(node.fieldName)))) {
        this.emit('new ');
      }
      this.visitType(node.fieldValue, level, ExceptionName, _name(node.fieldName));
  ...
  visitExceptions(ast, filepath, level) {
    ...
    for (let i = 0; i < exceptions.length; i++) {
      ...
      this.eachException(exceptions[i], level + 1);
      this.exceptionAfter();
      const modelFilePath = path.join(exDir, `${exceptionName}.cs`);
      this.save(modelFilePath);
    }
  }
  ...
  eachException(ast, level) {
    ...
    const exceptionName = _avoidReserveName(_name(ast.exceptionName));
    this.visitAnnotation(ast.annotation, level);
    let comments = DSL.comment.getFrontComments(this.comments, ast.tokenRange[0]);
    this.visitComments(comments, level);
  ...
  eachImport(imports, usedModels, innerModule, filepath, level) {
    ...
    for (let i = 0; i < imports.length; i++) {
      ...
      const moduleDir = this.config.libraries[aliasId];  // moduleDir为空说明libraries没有这个外部依赖，innerModule
...
`);
  ...
  visitAnnotation(annotation, level) {
    ...
    var deprecatedText = deprecated ? deprecated.text.text : '';
    ...
    if (deprecated) {
      if (deprecatedText !== '') {
        this.emit(`[Obsolete("${deprecatedText.trimEnd()}")]\n`, level);
      } else {
        this.emit(`[Obsolete]\n`, level);
      }
    }
  ...
  visitConsoleCSProj() {
    ...
    //添加对Tea的依赖
    dependenciesClass.push('Tea:1.1.3');
