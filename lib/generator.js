...
`;
  ...
  visitExpr(ast, level, env = {}) {
    if (ast.type === 'boolean') {
      ...
      if (env.fromIf && (ast.type === 'or' || ast.type === 'and') && ast.left.isAsync && ast.left.type === 'call' && wrapName && !wrapName.startsWith('$') && !this.builtin[wrapName]) {
        this.emit('(');
      }
      ...
      if (ast.right.left && ast.right.left.propertyPath) {
        ...
        // wrapName = _upperFirst(_name(ast.right.left.propertyPath[0]));
      }
      if (ast.type === 'or') {
        this.emit(' || ');
        if (env.fromIf && ast.right.isAsync && ast.right.type === 'call' && wrapName && !wrapName.startsWith('$') && !this.builtin[wrapName]) {
          this.emit('(');
        }
      ...
      if (ast.expr.type === 'call' && wrapName && !wrapName.startsWith('$') && !this.builtin[wrapName]) {
        let isEquals = false;
        if (ast.expr.left.propertyPath && ast.expr.left.propertyPath[0] && ast.expr.left.propertyPath[0].lexeme === 'equals') {
          isEquals = true;
        }
        if (isEquals) {
          this.emit(')');
