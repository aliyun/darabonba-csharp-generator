...
class Visitor {
    ...
    if (ast.type === 'array') {
      ...
      if(ast.moduleName) {
        ...
        if (this.conflictModels.get(modelMap)) {
          this.emit(`${this.imports[ast.moduleName].namespace}.Models.`);
        }
      } else {
        const modelMap = `${_name(ast)}`;
        if (this.conflictModels.get(modelMap)) {
          this.emit(`${this.namespace}.Models.`);
        }
      }
      let tempName = '';
      ...
      ast.path.forEach((item, index) => {
        if (index === 0) {
          ...
          moduleName = `${this.imports[_name(item)].namespace}.Models`;
          daraModuleName = _name(item);
        } else {
          //this.emit('.');
          //this.emit(fullName + _upperFirst(_name(item)));
          modelName += `.${fullName + _upperFirst(_name(item))}`;
          daraModelName += `.${_name(item)}`;
          fullName += _upperFirst(_name(item));
        }
      });
      
      const checkModel = daraModuleName + ':' + daraModelName.substring(1);
      if(this.conflictModels.get(checkModel)) {
        this.emit(moduleName + modelName);
      } else {
        this.emit(modelName.substring(1));
      }
      ...
      if(this.conflictModels.get(_name(ast))) {
  ...
  visitExpr(ast, level, env) {
    if (ast.type === 'boolean') {
      ...
      if (id === '__response') {
        ...
        let className = _avoidReserveName(id);
        if(this.conflictModels.get(id)){
          className = `${this.namespace}.Models.${_avoidReserveName(id)}`;
        }
        this.emit('typeof(' + className + ')');
      } else {
        this.emit(_avoidReserveName(id));
      }
  ...
  visitConstructModel(ast, level, env) {
    ...
    if (ast.propertyPath && ast.propertyPath.length > 0) {
      if (ast.inferred && ast.inferred.moduleName) {
        const modelMap = `${ast.inferred.moduleName}:${_name(ast.inferred)}`;
        if( this.conflictModels.get(modelMap)) {
          let moduleName = `${this.imports[_name(ast.aliasId)].namespace}.Models`;
          this.emit(`new ${moduleName}.`);
        } else {
          this.emit('new ');
        }
      } else {
        if(this.conflictModels.get(_name(ast.inferred))) {
          let moduleName = `${this.namespace}.Models`;
          this.emit(`new ${moduleName}.`);
        } else {
          this.emit('new ');
        }
      ...
      if(this.conflictModels.get(_name(ast.aliasId))) {
        let moduleName = `${this.namespace}.Models`;
        this.emit(`new ${moduleName}.`);
      } else {
        this.emit('new ');
  ...
  visitUsing() {
    for (var item of this.usedExternModel.entries()) {
      this.emit(`using ${this.imports[item[0]].namespace};\n`);
      if(item[1].size > 0) {
        this.emit(`using ${this.imports[item[0]].namespace}.Models;\n`);
      }
    }
  }
...
`);
  ...
  async visitConsoleCSProj() {
    ...
    //添加对Tea的依赖
    dependenciesClass.push('Tea:1.0.4');
