...
class Visitor {
  ...
  getRealClientName(aliasId) {
    ...
    if (!_isBuiltinNamespace(moduleInfo.namespace)) {
  ...
  getRealModelName(namespace, subModelName, type = 'Models') {
    ...
    if (namespace) {
      if (!_isBuiltinNamespace(namespace)) {
  ...
  _type(name) {
    ...
    if (name === '$Request' || name === '$Model' || name === '$Response' ||
      name === '$Date' || name === '$File' || name === '$URL') {
      return `Darabonba.${name.replace('$', '')}`;
    } else if (name === '$SSEEvent' || name === '$FileField' || name === '$RuntimeOptions' || name === '$ExtendsParameters') {
      return `Darabonba.Models.${name.replace('$', '')}`;
    } else if (name === '$RetryOptions') {
      return `Darabonba.RetryPolicy.${name.replace('$', '')}`;
    } else if (name === '$Bytes' || name === '$Form' || name === '$String' ||
      name === '$XML' || name === '$JSON' || name === '$Number' || name === '$Stream') {
      return `Darabonba.Utils.${name.replace('$', '')}Utils`;
    }

    if (name === '$ResponseError') {
      return 'Darabonba.Exceptions.DaraResponseException';
    }

    if (name === '$Error') {
      return 'Darabonba.Exceptions.DaraException';
    }
    return name;
  }
  ...
  getType(name) {
    ...
    } else if (name === '$RuntimeOptions' || name === '$ExtendsParameters' || name === '$SSEEvent' || name === 'FileField') {
...
`;
  ...
  visitModule(ast, filepath, main, level) {
    ...
    if (this.config.packageInfo.interface) {
      ...
      // for stock fuctions like isNull
    ...
    if (ast.catchBlocks && ast.catchBlocks.length > 0) {
      ast.catchBlocks.forEach(catchBlock => {
        ...
        if (!catchBlock.id.type) {
          const errorName = this.getRealModelName('Darabonba.Exceptions', 'Darabonba.Exceptions.DaraException');
  ...
  visitRuntimeBefore(ast, level, env) {
    ...
    const retryType = this.getRealModelName('Darabonba.RetryPolicy', 'Darabonba.RetryPolicy.RetryOptions');
    ...
    this.emit(';\n');
    this.emit('\n');
    this.emit('Darabonba.RetryPolicy.RetryPolicyContext _retryPolicyContext = null;\n', level);
    ...
    this.emit('_retryPolicyContext = new Darabonba.RetryPolicy.RetryPolicyContext\n', level);
    this.emit('{\n', level);
    this.emit('RetriesAttempted = _retriesAttempted\n', level + 1);
    this.emit('};\n', level);
    this.emit(`while (Darabonba.Core.ShouldRetry((${retryType})${RUNTIME}["retryOptions"], _retryPolicyContext))\n`, level);
    this.emit('{\n', level);
    this.emit('if (_retriesAttempted > 0)\n', level + 1);
    this.emit('{\n', level + 1);
    this.emit(`int backoffTime = Darabonba.Core.GetBackoffDelay((${retryType})${RUNTIME}["retryOptions"], _retryPolicyContext);\n`, level + 2);
    this.emit('if (backoffTime > 0)\n', level + 2);
    this.emit('{\n', level + 2);
    if (env.isAsyncMode) {
      this.emit('await Task.Delay(backoffTime);\n', level + 3);
    } else {
      this.emit('Thread.Sleep(backoffTime);\n', level + 3);
      this.used.push('System.Threading');
    }
    this.emit('}\n', level + 2);
    this.emit('}\n', level + 1);
    this.emit('try\n', level + 1);
  ...
  visitRuntimeAfter(ast, level) {
    ...
    this.emit('}\n', level + 1);
    this.emit('}\n\n', level);
    this.emit('throw Darabonba.Core.ThrowException(_retryPolicyContext);\n', level);
  }
  ...
  visitExpr(ast, level, env = {}) {
    if (ast.type === 'boolean') {
      ...
      env.groupOp = true;
      let wrapName;
      if (ast.left.left && ast.left.left.id) {
        wrapName = _upperFirst(_name(ast.left.left.id));
      }
      // sync call dose not need '('
      let isAsync = env.isAsyncMode && ast.left.isAsync;
      if (env.fromIf && (ast.type === 'or' || ast.type === 'and') && isAsync && ast.left.type === 'call' && wrapName && !wrapName.startsWith('$') && !this.builtin[wrapName]) {
        this.emit('(');
      }
      this.visitExpr(ast.left, level, env);
      if (env.fromIf && (ast.type === 'or' || ast.type === 'and') && ast.left.type === 'call' && wrapName && !wrapName.startsWith('$') && !this.builtin[wrapName]) {
        let isEquals = false;
        if (ast.right.left.propertyPath && ast.right.left.propertyPath[0]) {
          // instance_call class name, e.g. contains, length
          const method = (_name(ast.right.left.propertyPath[0]));
          if (ast.right.builtinModule && this.builtin[ast.right.builtinModule] && this.builtin[ast.right.builtinModule][method]) {
            isEquals = true;
          }
        }
        if (isAsync) {
      ...
      if (ast.right.left && ast.right.left.propertyPath) {
        wrapName = _name(ast.right.left.id);
      }
      isAsync = env.isAsyncMode && ast.right.isAsync;
      if (ast.type === 'or') {
        this.emit(' || ');
        if (env.fromIf && isAsync && ast.right.type === 'call' && wrapName && !wrapName.startsWith('$') && !this.builtin[wrapName]) {
          this.emit('(');
        }
      ...
      // wrapName.startsWith('$') is funcs like $equals, $default
      if (env.fromIf && (ast.type === 'or' || ast.type === 'and') && ast.right.type === 'call' && wrapName && !wrapName.startsWith('$') && !this.builtin[wrapName]) {
        let isEquals = false;
        if (ast.right.left.propertyPath && ast.right.left.propertyPath[0]) {
          // instance_call class name, e.g. contains, length
          const method = (_name(ast.right.left.propertyPath[0]));
      ...
      let needValue = false;
      const isAsync = env.isAsyncMode && ast.expr.isAsync;
      if (env.fromIf && (ast.expr.type === 'group' || ast.expr.type === 'call') && wrapName && !wrapName.startsWith('$') && !this.builtin[wrapName]) {
        if ((isAsync || !ast.expr.builtinModule)) {
          needValue = true;
          needBrackets = true;
        } else if (ast.expr.builtinModule && ast.expr.builtinModule === '$String') {
          if (ast.expr.left.propertyPath && ast.expr.left.propertyPath[0]) {
            const method = (_name(ast.expr.left.propertyPath[0]));
            if (method === 'equals') {
              // only $String.equals need brackets because of == operator
              needBrackets = true;
            } else if (ast.expr.builtinModule && this.builtin[ast.expr.builtinModule] && this.builtin[ast.expr.builtinModule][method]) {
              needBrackets = false;
            }
          }
        }
  ...
  visitAssign(ast, level, env) {
    ...
    if (ast.expr.needToReadable) {
      this.emit('Darabonba.Utils.StreamUtils.BytesReadable(');
    }
  ...
  visitThrow(ast, level, env) {
    ...
    if (ast.expr.type === 'construct_model') {
      ...
      this.emit(`new Darabonba.Exceptions.DaraException(`);
  ...
  visitObject(ast, level, env) {
    ...
    this.emit(`Darabonba.Utils.ConverterUtils.Merge<${this._type(ast.inferred.valueType.name)}>\n`);
    this.emit('(\n', level);
  ...
  visitExtendOn(extendOn, type = 'model') {
    if (!extendOn) {
      ...
      this.emit('Darabonba.Exceptions.DaraException');
      return;
    }
    let namespace = this.namespace;
    ...
    if (extendOn.type === 'moduleModel') {
      ...
      if (extendOn.lexeme === '$ResponseError') {
        this.emit(this.getRealModelName('Darabonba.Exceptions', 'Darabonba.Exceptions.DaraResponseException'));
      }
      if (extendOn.lexeme === '$Error') {
        this.emit(this.getRealModelName('Darabonba.Exceptions', 'Darabonba.Exceptions.DaraException'));
      }
    } else {
  ...
  eachAPI(ast, level, env = {}) {
    ...
    if (env.isAsyncMode) {
      this.emit(`Darabonba.Response ${RESPONSE} = await Darabonba.Core.DoActionAsync(${REQUEST}`, baseLevel + 1);
    } else {
      this.emit(`Darabonba.Response ${RESPONSE} = Darabonba.Core.DoAction(${REQUEST}`, baseLevel + 1);
    }
