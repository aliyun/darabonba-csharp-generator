...
`;
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if (node.fieldValue.fieldType === 'array') {
        ...
        if (node.fieldValue.fieldType.idType === 'module') {
          ...
          const realModelName = this.getRealModelName(this.getType(node.fieldValue.lexeme));
          this.emit(realModelName);
    ...
    if (ast.type === 'array') {
      ...
      models.forEach((item, index) => {
        ...
        if (index === 0) {
          ...
          const realModelName = this.getRealModelName(`${namespace}.${type}.${tempName}`);
          this.emit(realModelName);
        } else {
          tempName += _upperFirst(_type(item));
          this.emit(tempName);
      ...
      const subModelName = this.getRealModelName(`${namespace}.${type}.${moduleModelName}`);
      this.emit(subModelName);
      ...
      const realModelName = this.getRealModelName(`${this.namespace}.${type}.${_upperFirst(ast.lexeme)}`);
      this.emit(realModelName);
  ...
  visitConstructModel(ast, level, env) {
    ...
    if (ast.aliasId.isModule) {
      ...
      if (_isBuiltinModel(modelName)) {
        ...
        const subModelName = this.getRealModelName(`${this.namespace}.${type}.${modelName}`);
        this.emit(subModelName);
      }
    } else {
  ...
  eachException(ast, level, env) {
    ...
    let comments = DSL.comment.getFrontComments(this.comments, ast.tokenRange[0]);
    this.visitComments(comments, level);
    this.visitException(ast, exceptionName, ast.extendOn, level, env);
  }
