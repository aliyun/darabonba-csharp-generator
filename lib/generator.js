...
`;
  ...
  visitType(ast, level, env) {
    if (ast.type === 'array') {
      ...
      this.emit(_type(_name(ast), env && env.variable));
    } else {
      this.emit(_type(_name(ast), env && env.variable));
    }
  }
  ...
  visitExpr(ast, level, env = {}) {
    if (ast.type === 'boolean') {
      ...
      this.emit('!');
      this.visitExpr(ast.expr, level, env);
      if (ast.expr.type !== 'call' && ast.expr.inferred.type !== 'basic') {
        this.emit('.Value');
      }
  ...
  visitIf(ast, level, env = {}) {
    ...
    if (ast.condition.type === 'variable' && ast.condition.inferred.type !== 'basic') {
      this.emit('.Value');
    }
    this.emit(')\n');
    this.emit('{\n', level);
