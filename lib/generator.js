...
const {
  _name, _upperFirst, _string, _type, _lowerFirst, _subModelName,
...
class Visitor {
  constructor(option = {}) {
    this.config = Object.assign({
      outputDir: '',
      indent: '    ',
      clientPath: 'Client.cs'
    }, option);
    assert.ok(this.config.outputDir, '`option.outputDir` should not empty');
    assert.ok(this.config.namespace, '`option.namespace` should not empty');
  ...
  visitModule(ast, level) {
    ...
    if (ast.extends) {
      ...
      extendParam.extendClass = this.imports[extendsName].className || 'Client';
      extendParam.extendNamespace = this.imports[extendsName].namespace;
    }
  ...
  visitModelBody(ast, level, env, modelName) {
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if (_isSystemProperty(paramName)) {
        publicDeclar = publicDeclar + 'new ';
      }
  ...
  visitModelField(ast, level, modelName, isObject) {
    ...
    if (isObject === undefined) {
      ...
      if (ast.fieldValue.fieldType === undefined) {
          ...
          if (ast.fieldValue.fieldItemType.nodes !== undefined) {
            ...
            if (ast.fieldValue.fieldItemType.nodes[0].fieldValue.type === 'modelBody') {
              ...
              ast.fieldValue.fieldItemType.nodes.forEach(cNode => {
                this.visitModelField(cNode, level + 2, subModelName);
              });
            }
            this.emit('}\n', level);
          } else {
  ...
  visitType(ast, level, env) {
    if (ast.type === 'array') {
      ...
      for (let i = 0; i < ast.path.length; i++) {
        ...
        if (ast.realType === 'moduleModel' && i === 0) {
        ...
        tempName += _upperFirst(_name(item));
        this.emit(tempName);
      }
  ...
  visitStmt(ast, level, env) {
    ...
    } else if (ast.type === 'requestAssign') {
      this.visitRequestAssign(ast, level, env);
    ...
    } else if (ast.type === 'ifRequestAssign') {
      this.visitIfRequestAssign(ast, level, env);
  ...
  visitTry(ast, level, env) {
    ...
    if (ast.catchBlock) {
      ...
      if (ast.catchId) {
        this.emit(`(Exception ${_name(ast.catchId)})`);
      }
      this.emit('\n');
      this.emit('{\n', level);
  ...
  visitIfRequestAssign(ast, level, env) {
    this.emit('if (', level);
    this.visitExpr(ast.condition, level, env);
    this.emit(')\n');
    this.emit('{\n', level);
    for (let i = 0; i < ast.assigns.length; i++) {
      this.visitRequestAssign(ast.assigns[i], level + 1, env);
    }
    this.emit('}\n', level);


    if ((!ast.elseIfs || ast.elseIfs.length === 0) && !ast.elseAssigns) {
      return;
    }

    if (ast.elseIfs.length > 0) {
      ast.elseIfs.forEach((branch) => {
        this.emit('else if (');
        this.visitExpr(branch.condition, level, env);
        this.emit(')\n');
        this.emit('{\n', level);
        for (let i = 0; i < branch.assigns.length; i++) {
          this.visitRequestAssign(branch.assigns[i], level + 1, env);
        }
        this.emit('}\n', level);
      });
    }
    if (ast.elseAssigns) {
      this.emit('else\n');
      this.emit('{\n', level);
      for (let i = 0; i < ast.elseAssigns.length; i++) {
        this.visitRequestAssign(ast.elseAssigns[i], level + 1, env);
      }
      this.emit('}\n', level);
    }
  }

  visitRequestAssign(ast, level, env) {
    if (ast.type === 'throw') {
      this.visitThrow(ast, level, env);
      return;
    }
    var name = _name(ast.left.id);
    var requestModel = this.predefined['$Request'];
    var def = requestModel.modelBody.nodes.find((item) => {
      return item.fieldName.lexeme === name;
    });

    this.emit(`${REQUEST}`, level);
    var expectedType = def.fieldValue;

    if (ast.left.type === 'id') {
      this.emit(`.${_upperFirst(name)}`);
    } else if (ast.left.type === 'request_property_assign') {
      this.emit(`.${_upperFirst(name)}`);
      for (var i = 0; i < ast.left.propertyPath.length; i++) {
        if (def.fieldValue.fieldType === 'map') {
          this.emit(`["${_name(ast.left.propertyPath[i])}"]`);
        } else {
          this.emit(`.${_lowerFirst(ast.left.propertyPath[i])}`);
        }

        expectedType = expectedType.valueType.lexeme;
      }
    } else {
    ...
    this.emit(' = ');
    const expected = ast.left.inferred;
    const actual = ast.expr.inferred;
    let needToReadable = false;
    if (expected.type === 'basic' && expected.name === 'readable') {
      if (actual.type === 'basic' && actual.name === 'bytes' ||
        actual.type === 'basic' && actual.name === 'string') {
        // inferred: { type: 'basic', name: 'readable' }
        // inferred: { type: 'basic', name: 'bytes' }
        needToReadable = true;
      }
    }
    if (needToReadable) {
    ...
    this.visitExpr(ast.expr, level, env, expectedType);
    if (needToReadable) {
      this.emit(')');
    }
    this.emit(';\n');
  ...
  visitExpr(ast, level, env) {
    if (ast.type === 'boolean') {
      ...
      this.visitVirtualCall(ast, level, env);
    ...
    } else if (ast.type === 'module_call') {
      this.visitModuleCall(ast, level, env);
  ...
  visitVirtualCall(ast, level, env) {
  ...
  eachSubModel(ast, level, predefined) {
    assert.equal(ast.type, 'model');
    const env = {
      predefined
    };
    const modelName = _subModelName(_name(ast.modelName));
    this.emit(`protected class ${modelName} {\n`, level);
    this.visitModelBody(ast.modelBody, level + 1, env, modelName);
    this.emit('}\n\n', level);
  }
  ...
  eachAPI(ast, level, env) {
    ...
    for (var i = 0; i < ast.params.params.length; i++) {
      ...
      if (_name(param.paramType) && !DSL.util.isBasicType(_name(param.paramType))) {
        this.emit(`${param.paramName.lexeme}.Validate();\n`, level + 1);
      }
    }
  ...
  eachImport(imports, usedModels, level) {
    ...
    for (let i = 0; i < imports.length; i++) {
      ...
      let csharpConfig = teaFile.csharp;
      teaFile.releases = teaFile.releases || {};
      if (!csharpConfig) {
        throw new Error(`The '${aliasId}' has no csharp supported.`);
      }
      csharpConfig.release = teaFile.releases.csharp || csharpConfig.namespace + ':0.0.1';
...
namespace ${this.namespace}
{
    ...
    if (extendParam.extendClass) {
      this.emit(`: ${extendParam.extendNamespace}.${extendParam.extendClass}
    {
`);
    } else {
      this.emit(`
...
`);
  ...
  visitInit(ast, types, level, env) {
    ...
    let comments = DSL.comment.getFrontComments(this.comments, ast.tokenRange[0]);
    this.visitComments(comments, level + 2);
    this.emit('public Client', level + 2);
