...
const nullableTypes = ['number', 'boolean', 'integer', 'uint16', 'int16', 'uint8', 'int8', 'uint32', 'int32', 'uint64', 'int64', 'float', 'double', 'long', 'ulong'];
...
function _type(name, variable = false) {
  if (variable) {
    return notNullTypeMap[name] || typeMap[name] || name;
  }
...
function splitByLastDot(str) {
  ...
  const part1 = str.substring(0, lastDotIndex);
  const part2 = str.substring(lastDotIndex + 1);
...
module.exports = {
  _name, _upperFirst, _string, _prop, _type, _lowerFirst, _isBinaryOp,
