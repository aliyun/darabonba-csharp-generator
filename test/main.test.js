...
function compareDirectories(expectedDir, outputDir, moduleName) {
  ...
  for (let fileName of expectedFiles) {
    ...
    if (fileName === 'Properties' || fileName === 'bin' || 
      fileName === 'obj' || fileName === '.idea' || fileName === '.vscode' || fileName === 'global.json') {
      continue;
    }
    if (!outputFiles.includes(fileName)) {
      console.log(`${moduleName} expected file ${fileName} not found`);
      assert.ok(false);
    }
...
describe('new Generator', function () {
  ...
  it('transition should ok', function () {
    const pkgContent = fs.readFileSync(path.join(__dirname, 'fixtures/transition/Darafile'), 'utf8');
    const pkg = JSON.parse(pkgContent);
    check('transition', {
      pkgDir: path.join(__dirname, 'fixtures/transition'),
      libraries: pkg.libraries,
      exec: true,
      ...pkg.csharp,
      editable: true
